...
// Get ID card from props (dashboard) or from shared page props (middleware)
const teacherIdCard = computed(() => {
    return props.idCard || page.props.teacher_id_card || null;
});

const currentTeacher = computed(() => {
    try {
        const teacherData = page.props.current_teacher;
...
const logout = () => {
    if (confirm('Are you sure you want to logout?')) {
        router.post(route('teacher.logout'));
    }
};
...
const toggleIdCardModal = () => {
    ...
    if (showIdCardModal.value && teacherIdCard.value?.card_number) {
        generateQRCode();
    }
};
...
const generateQRCode = async () => {
    if (!teacherIdCard.value?.card_number) return;
    
    try {
        const dataUrl = await QRCode.toDataURL(teacherIdCard.value.card_number, {
            width: 200,
            margin: 2,
            color: {
                dark: '#000000',
...
watch(() => showIdCardModal.value, (isOpen) => {
    if (isOpen && teacherIdCard.value?.card_number) {
        nextTick(() => {
            generateQRCode();
        });
    }
...
<template>
    <div :class="{ 'dark': isDark }">
        <div class="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-50 dark:from-gray-900 dark:via-slate-900 dark:to-gray-900 transition-all duration-300">
            <!-- Header -->
                ...
                <div class="px-6 py-4 flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        >
                            ...
                            <span v-if="teacherIdCard?.status === 'active'" class="absolute top-0 right-0 w-2 h-2 bg-green-500 rounded-full"></span>
                        </button>
                        ...
                        <div class="relative profile-menu-container">
                            ...
                            >
                                ...
                                <span class="hidden sm:inline text-sm font-medium text-gray-700 dark:text-gray-300">{{ currentTeacher?.user?.full_name || "Teacher" }}</span>
                            ...
                            >
                                <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                                    <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ currentTeacher?.user?.full_name || "Teacher" }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ currentTeacher?.user?.email || "" }}</p>
                                </div>
                                <div class="p-2">
                                    <button
                                        @click="logout"
                    ...
                    <div class="flex items-center gap-4">
                        <Link
                            ...
                            :class="[
                                ...
                                $page.url.startsWith('/teacher/dashboard')
                        ...
                        <Link
                            ...
                            :class="[
                                'px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-2',
                                $page.url.startsWith('/teacher/grades')
                                    ? 'bg-blue-500 text-white shadow-lg'
                                    : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'
                            ]"
                        >
                        ...
                        <Link
                            ...
                            :class="[
                                'px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-2',
                                $page.url.startsWith('/teacher/assessments')
                                    ? 'bg-blue-500 text-white shadow-lg'
                                    : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'
                            ]"
                        >
                        ...
                        <Link
                            ...
                            :class="[
                                'px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-2',
                                $page.url.startsWith('/teacher/classes')
                                    ? 'bg-blue-500 text-white shadow-lg'
                                    : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'
                            ]"
                        >
                        ...
                        <Link
                            ...
                            :class="[
                                'px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-2',
                                $page.url.startsWith('/teacher/students')
                                    ? 'bg-blue-500 text-white shadow-lg'
                                    : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'
                            ]"
                        >
            ...
            <!-- ID Card Modal -->
            <div 
                v-if="showIdCardModal && teacherIdCard"
                    ...
                    <div class="bg-gradient-to-r from-purple-500 to-pink-500 p-6 text-white">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="text-xl font-bold">Teacher ID Card</h3>
                                    <p class="text-sm text-white/80">{{ teacherIdCard?.card_number }}</p>
                                </div>
                            </div>
                            <button
                    ...
                    <div class="p-6">
                        <div class="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-gray-700 dark:to-gray-800 rounded-xl p-6 border-2 border-purple-200 dark:border-purple-800">
                            ...
                            <div class="text-center mb-6">
                                ...
                                <p class="text-sm text-purple-700 dark:text-purple-300">TEACHER IDENTIFICATION CARD</p>
                            </div>
                            ...
                            <div class="flex gap-4">
                                ...
                                <div class="flex-shrink-0">
                                    <div class="w-24 h-24 bg-white dark:bg-gray-700 rounded-lg border-2 border-purple-300 dark:border-purple-600 overflow-hidden flex items-center justify-center">
                                        <img 
                                            v-if="teacherIdCard?.photo_path" 
                                            :src="`/storage/${teacherIdCard.photo_path}`" 
                                            alt="Teacher Photo"
                                ...
                                <div class="flex-1 space-y-2">
                                    <div>
                                        ...
                                        <p class="text-lg font-bold text-purple-900 dark:text-purple-100">{{ currentTeacher?.user?.full_name || 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-purple-600 dark:text-purple-400 font-medium">EMPLOYEE ID</p>
                                        <p class="text-base font-semibold text-purple-900 dark:text-purple-100">{{ currentTeacher?.employee_id || 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-purple-600 dark:text-purple-400 font-medium">CARD NUMBER</p>
                                        <p class="text-base font-mono font-semibold text-purple-900 dark:text-purple-100">{{ teacherIdCard?.card_number }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 pt-4 border-t border-purple-200 dark:border-purple-700">
                                <div class="flex justify-between text-xs text-purple-600 dark:text-purple-400">
                                    <div>
                                        ...
                                        <p>{{ teacherIdCard?.issue_date ? new Date(teacherIdCard.issue_date).toLocaleDateString() : 'N/A' }}</p>
                                    </div>
                                    <div v-if="teacherIdCard?.expiry_date">
                                        <p class="font-medium">EXPIRY DATE</p>
                                        <p>{{ new Date(teacherIdCard.expiry_date).toLocaleDateString() }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 flex justify-center">
                                <span :class="[
                                    ...
                                    teacherIdCard?.status === 'active' 
                                        ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300'
                                        : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300'
                                ]">
                                    <i :class="teacherIdCard?.status === 'active' ? 'fas fa-check-circle' : 'fas fa-exclamation-circle'"></i>
                                    {{ teacherIdCard?.status ? teacherIdCard.status.charAt(0).toUpperCase() + teacherIdCard.status.slice(1) : 'N/A' }}
                                </span>
                            </div>
