...
const title = [{label: "Teachers", href: route("admin.teachers.index")}];
...
const handleSearch = () => {
    router.get(route('admin.teachers.index'), { search: searchQuery.value }, {
        preserveState: true,
        preserveScroll: true
    });
};
...
<template>
    <App :title="title">
        <AdminPageWrapper
            ...
            :subtitle="`Total: ${teachers?.total || 0} teachers`"
        ...
        >
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Employee ID</th>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Specialization</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        >
                            ...
                            <td class="px-6 py-4">
                                <div>
                                    <span class="text-gray-800 dark:text-gray-200 font-medium">{{ teacher.user?.full_name || 'N/A' }}</span>
                                    <span v-if="teacher.user?.username" class="text-gray-600 dark:text-gray-400 text-xs block">@{{ teacher.user.username }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <tr v-if="!teachers?.data || teachers.data.length === 0">
                            <td colspan="7" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center">
                                    <i class="fas fa-chalkboard-teacher text-4xl mb-2 text-gray-300 dark:text-gray-600"></i>
            ...
            <Pagination v-if="teachers?.links" :links="teachers.links"/>
        </AdminPageWrapper>
    </App>
</template>
