...
const title = [
    ...
    {label: "Edit Timetable Entry", href: "#"}
];
...
const form = useForm({
    class_id: props.timetable?.class_id || '',
    subject_id: props.timetable?.subject_id || '',
    teacher_id: props.timetable?.teacher_id || '',
    day_of_week: props.timetable?.day_of_week || 'monday',
    start_time: props.timetable?.start_time ? props.timetable.start_time.substring(0, 5) : '',
    end_time: props.timetable?.end_time ? props.timetable.end_time.substring(0, 5) : '',
    room: props.timetable?.room || '',
    academic_year: props.timetable?.academic_year || '',
    status: props.timetable?.status || 'active',
    notes: props.timetable?.notes || '',
});
...
watch(() => form.class_id, () => {
    ...
    if (selectedClass.value) {
        const subjectExists = availableSubjects.value.some(s => s.id == form.subject_id);
        const teacherExists = availableTeachers.value.some(t => t.id == form.teacher_id);
...
<template>
    <App :title="title">
        <AdminPageWrapper
            ...
            subtitle="Update timetable entry information"
            icon="fas fa-edit"
            ...
            <form @submit.prevent="submit" class="p-6 space-y-6">
                ...
                <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button
                        ...
                        class="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-6 py-2 rounded-lg transition-all duration-200 flex items-center gap-2 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <i v-if="form.processing" class="fas fa-spinner fa-spin"></i>
                        <i v-else class="fas fa-save"></i>
                        {{ form.processing ? 'Updating...' : 'Update Timetable Entry' }}
                    </button>
                </div>
            </form>
        </AdminPageWrapper>
